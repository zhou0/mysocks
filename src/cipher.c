...
#include <unistd.h>
        ...
        double d1 = drand48();
        double d2 = drand48();
        memcpy(cipher.encrypt.iv.base, &d1, sizeof (d1));
        memcpy(cipher.encrypt.iv.base + sizeof (d1), &d2, sizeof (d2));
#endif
#endif
        arcfour_setkey(&cipher.encrypt.ctx, create_key(cipher.encrypt.iv.base, cipher.encrypt.iv.len), cipher.keyl);
        /*
#if defined(NDEBUG)
#else
...
unsigned char * cipher_decrypt(conn *c, size_t * plainl, char * encrypt, size_t encryptl)
{
    ...
    if (!c->request.len)
    {
        ...
        if (strcmp(config.method, "rc4") == 0)
        {
            ...
            arcfour_setkey(&cipher.decrypt.ctx, create_key(cipher.decrypt.iv.base, cipher.decrypt.iv.len), cipher.keyl);
        }
