...
ARG MYSOCKS_URL=https://github.com/zhou0/mysocks/archive/0.2.tar.gz
...
RUN set -ex && \
    ...
    curl -sSL $MYSOCKS_URL | tar xz && cd mysocks-0.2/build/debug && \
    rm CMakeCache.txt && \
    ...
    apk del .build-deps && \
    rm -fr libuv-1.11.0 && rm -fr mysocks-0.2 
