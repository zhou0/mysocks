...
ARG MYSOCKS_URL=https://github.com/zhou0/mysocks/archive/0.5.8.tar.gz
...
make && make install && cd .. && \
    curl -sSL $MYSOCKS_URL | tar xz && cd mysocks-0.5.8 && mkdir -p \
build/release && cd build/release && \
    ...
    apk del .build-deps && \
    rm -fr libuv-1.14.0 && rm -fr wolfssl-3.11.0-stable && rm -fr mysocks-0.5.8
